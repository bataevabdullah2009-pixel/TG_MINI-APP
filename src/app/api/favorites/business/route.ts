...
export async function GET(request: NextRequest) {
  const finishTiming = createServerTiming("favorite_businesses");
  try {
    ...
    return finishTiming(NextResponse.json({ ok: false, error: "Избранное временно недоступно." }, { status: 500 }));
  }
}
